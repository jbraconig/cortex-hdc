...
- [x] **Streaming / Mini-batch K-Means**: Modify the training phase (`trainer.go`) to avoid accumulating all `HVector`s in memory. Implement streaming or mini-batch K-Means to keep a predictable memory footprint when training with millions of log lines.
